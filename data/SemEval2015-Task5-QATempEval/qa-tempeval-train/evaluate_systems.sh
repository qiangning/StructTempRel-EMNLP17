...
echo "Using $PARTICIPANTION_PATH as path to find participations to evaluate." | tee $SCRIPT_PATH/results/log.txt
echo -e "RESULTS\n-------\nparticipant_system_name\t\tquests\tanswrd\tcorr\tinc\tacc\tprec\trec\tf1" > $SCRIPT_PATH/results/results.txt
...
# >>>>> Normalize participant TimeML with question-events
	...
	for folder in `ls -d ${PARTICIPANTION_PATH}systems/*/`;do
		if [[ "$folder" != "${PARTICIPANTION_PATH}systems/input/" && $folder != *-normalized* ]];then
			echo -e "\n>>>> Processing $folder" | tee -a $SCRIPT_PATH/results/log.txt
			...
			java -jar tools/timeml-normalizer/timeml-normalizer-1.1.0.jar -a "${PARTICIPANTION_PATH}eval-questions/question-events/;$folder" -respect $SCRIPT_PATH/results/log.txt >> $SCRIPT_PATH/results/log.txt 2>&1
			...
			java -jar tools/timeml-qa/timeml-qa-1.0.2.jar -a TQA ${folder%?}-normalized/question-set.txt 2>> $SCRIPT_PATH/results/log.txt > $SCRIPT_PATH/results/$system_run_name-results.txt
			echo -e "\tOutput: $SCRIPT_PATH/results/$system_run_name-results.txt"
			#rm -rf ${folder%?}-normalized
			results=`cat $SCRIPT_PATH/results/$system_run_name-results.txt | grep "^questions" | sed -e "s/[^=]*=\([^[:blank:]]*\)/	\1/g"` # Mac sed does not support \t so we use a normal tab
			echo -e "${system_run_name}\t$results" >> $SCRIPT_PATH/results/results.txt
			rm -rf ${folder%?}-normalized/question-set.txt
		fi
	done
	echo -e "\n\n--------------------------------------------"
	cat $SCRIPT_PATH/results/results.txt
	echo "--------------------------------------------"
	echo -e "\n\nLOG:$SCRIPT_PATH/results/log.txt\nRESULTS: $SCRIPT_PATH/results/results.txt\n"
