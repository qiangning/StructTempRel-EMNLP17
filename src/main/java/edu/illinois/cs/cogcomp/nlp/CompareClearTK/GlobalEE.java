...
public class GlobalEE {
    ...
    public List<TemporalDocument> ClearTKOutput;
    ...
    public static boolean use_cleartk_et = true;
    public boolean serialize = true;
    public String cacheDir = "serialized_data/aug_Bethard/global/";
    public boolean force_update = true;
    public double kl_threshold = 0;
    public boolean sentDistFilter = true;
    public int sentDist = 2;
    ...
    public TemporalDocument solveDoc(TemporalDocument doc) throws Exception {
        ...
        if(sentDistFilter) {
            ...
            for (TLINK tlink : predDoc.getBodyTlinks()) {
                ...
                if(Math.abs(sentId1-sentId2)>=sentDist)
                    continue;
                filtered.add(tlink);
            }
        ...
        for (int i = 0; i < n; i++) {
            ...
            try{
                ...
                if(use_cleartk_et) {
                    ...
                    for (TemporalDocument cleartkdoc : ClearTKOutput) {
                        if (cleartkdoc.getDocID().equals(predDoc.getDocID())) {
                            tlinks.addAll(cleartkdoc.getETlinks());
                            break;
                        }
                    }
                }
    ...
    public static void main(String[] args) throws Exception{
        ...
        List<TemporalDocument> testdocs = TempEval3Reader.deserialize(TempEval3Reader.label2dir("platinum"));

        GlobalEE exp;
        String dir,tag;
        if(!postfilter) {
            exp = new GlobalEE(testdocs,
                    rm2.getString("eeModelDirPath") + "CompClearTK/",
                    rm2.getString("eeModelName_none") + "_0.1_TB_Bethard");
            exp.cacheDir = "serialized_data/aug_Bethard/global/";
            exp.kl_threshold = 0;
            dir = "./output/ClearTK/global";
            tag = "cleartk_vs_global";
        }
        else {
            exp = new GlobalEE(testdocs,
                    rm2.getString("eeModelDirPath") + "CompClearTK/",
                    rm2.getString("eeModelName") + "_TB_Bethard");
            exp.cacheDir = "serialized_data/aug_Bethard/global-postfilter/";
            exp.kl_threshold = 1.2;
            dir = "./output/ClearTK/global-postfilter";
            tag = "cleartk_vs_global-pf";
        }
        ...
        String cmd = "sh scripts/evaluate_general.sh ./data/TempEval3/Evaluation/te3-platinum "+dir+" "+tag;
